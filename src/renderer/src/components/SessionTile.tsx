...
const sessionTerminalGeometryCache = new Map<
  string,
...
function writeConflictSummary(
  ...
  for (const conflict of conflicts.slice(0, 8)) {
    enqueueOutput(
      `\x1b[38;2;143;165;184mconflict:\x1b[0m ${conflict.path}\r\n`,
    );
  }
  if (conflicts.length > 8) {
    enqueueOutput(
      `\x1b[38;2;143;165;184m...and ${conflicts.length - 8} more\x1b[0m\r\n`,
    );
  }
}
...
export function SessionTile({
  ...
  const isCloudSession = session.mode === "cloud";
  ...
  const [viewMode, setViewMode] = useState<"terminal" | "history" | "review">(
    "terminal",
  );
  ...
  function performTerminalFit(): void {
    ...
    const hostChanged =
      lastGeometry.width !== width || lastGeometry.height !== height;
    const cellGeometryChanged =
      lastGeometry.cols !== cols || lastGeometry.rows !== rows;
  ...
  function handleWheel(event: React.WheelEvent<HTMLDivElement>): void {
    ...
    const lines =
      ...
      Math.max(1, Math.ceil(Math.abs(event.deltaY) / 48));
  ...
  useEffect(() => {
    ...
    const initializeTerminal = () => {
      ...
      const { replayData, unsubscribe: outputCleanup } = attachSessionOutput(
        session.id,
        (data) => {
          if (!primed) {
    ...
    const idleHandle = supportsIdleCallback
      ? ((window as any).requestIdleCallback(initializeTerminal, {
          timeout: 120,
        }) as number)
  ...
  useEffect(() => {
    ...
    if (session.error)
      enqueueOutput(`\x1b[38;2;143;165;184m${session.error}\x1b[0m\r\n`);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const root = await window.sentinel.readFile(
          `${session.projectRoot}${sep}${reviewFile}`,
        );
        const wt = await window.sentinel.readFile(
          `${session.workspacePath}${sep}${reviewFile}`,
        );
        if (!active) return;
  ...
  async function handleOp(op: "apply" | "commit" | "discard") {
    ...
    if (isCloudSession) {
      enqueueOutput(
        "\r\n\x1b[38;2;143;165;184mCloud sessions run in isolated server-side workspaces. Use git inside this terminal to commit, merge, or push changes back to your main branch.\x1b[0m\r\n",
      );
      return;
    }
    setOpLoading(op);
    try {
      if (op === "apply") {
        ...
        if (result.remainingPaths.length > 0) {
          enqueueOutput(
            `\x1b[38;2;143;165;184m${result.remainingPaths.length} change(s) still remain in the workspace.\x1b[0m\r\n`,
          );
        }
      ...
      if (op === "commit") {
        ...
        if (result.createdCommit) {
          const target =
            session.workspaceStrategy === "sandbox-copy"
              ? "the main project"
              : "the worktree branch";
          const hashSuffix = result.commitHash
            ? ` as ${result.commitHash}`
            : "";
          enqueueOutput(
        ...
        if (
          ...
          enqueueOutput(
            `\x1b[38;2;143;165;184m${result.appliedPaths.length} sandbox file(s) were synced into the main project.\x1b[0m\r\n`,
          );
        }
        if (result.remainingPaths.length > 0) {
          enqueueOutput(
  ...
  const canCommitSession =
    !isCloudSession && session.workspaceStrategy === "git-worktree";
  const commitTitle = isCloudSession
    ? "Use git inside the cloud terminal to commit or push changes"
    : "Commit Worktree Changes";
  const filteredHistory = historyQuery.trim()
    ? historyEntries.filter((e) =>
        e.command.toLowerCase().includes(historyQuery.toLowerCase()),
      )
    : historyEntries;
  ...
  return (
    <article
      ...
      onMouseDown={() => {
        if (viewMode === "terminal") scheduleTerminalFocus();
      }}
    >
      ...
      <div className="shrink-0 flex items-center justify-between border-b border-white/10 bg-black/40 px-2 h-[20px]">
        ...
        >
          {/* View toggles */}
          <button
            className={`px-1.5 py-0.5 text-[9px] uppercase tracking-widest transition ${viewMode === "terminal" ? "text-white" : "text-white/30 hover:text-white/70"}`}
          ...
          <button
            className={`px-1.5 py-0.5 text-[9px] uppercase tracking-widest transition ${viewMode === "review" ? "text-emerald-400" : "text-white/30 hover:text-white/70"}`}
          ...
          <button
            className={`px-1.5 py-0.5 text-[9px] uppercase tracking-widest transition ${viewMode === "history" ? "text-sentinel-accent" : "text-white/30 hover:text-white/70"}`}
          ...
          {canCommitSession && (
            <button
              ...
              disabled={
                isClosing || opLoading !== null || modifiedPaths.length === 0
              }
          ...
          <button
            ...
            onClick={() => handleOp("discard")}
            title={
              isCloudSession
                ? "Cloud sessions keep changes in their isolated workspace until you handle them with git in the terminal."
                : "Discard"
            }
          >
          ...
          <button
            className="px-1 text-white/30 hover:text-sentinel-glow transition disabled:opacity-20"
            disabled={
              isCloudSession ||
              isClosing ||
              opLoading !== null ||
            ...
            onClick={() => handleOp("apply")}
            title={
              isCloudSession
                ? "Cloud sessions only affect the main branch when you merge or push from the isolated workspace."
                : session.workspaceStrategy === "sandbox-copy"
                ? "Sync to Main Project Files"
                : "Merge to Main"
            }
          >
            {session.workspaceStrategy === "sandbox-copy" ? (
          ...
          <div className="mx-1 h-3 w-px bg-white/10" />
          <button
            className="px-1 text-white/30 hover:text-sentinel-accent transition disabled:opacity-20"
            disabled={isClosing}
            onClick={() => requestTerminalRebuild(0)}
            title="Recover display"
          >
            <RefreshCw className="h-2.5 w-2.5" />
          </button>
          <button
            className="px-1 text-white/30 hover:text-white transition"
      ...
      <div className="relative flex-1 min-h-0 overflow-hidden">
        {/* Terminal */}
        <div
          className={`absolute inset-0 ${viewMode === "terminal" ? "z-10" : "z-0 pointer-events-none"}`}
        >
          {isTerminalLive ? (
            <div
            ...
            <div className="flex h-full items-center justify-center bg-[#04070b] p-6 text-center">
              <div className="max-w-sm space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  {session.error && (
                    <p className="text-[11px] leading-5 text-rose-300/90">
                      {session.error}
                    </p>
                  )}
                </div>
        ...
        {/* Review / Diff */}
        <div
          className={`absolute inset-0 flex flex-col ${viewMode === "review" ? "z-10" : "z-0 pointer-events-none opacity-0"}`}
        >
          <div className="shrink-0 bg-black/20 px-2 py-1 border-b border-white/10">
            ...
            >
              {modifiedPaths.length === 0 && (
                <option value="">No modified files</option>
              )}
              {modifiedPaths.map((p) => (
        ...
        {/* History */}
        <div
          className={`absolute inset-0 flex flex-col ${viewMode === "history" ? "z-10" : "z-0 pointer-events-none opacity-0"}`}
        >
          ...
          <div className="flex-1 min-h-0 overflow-auto p-1.5 space-y-0.5">
            {filteredHistory.map((entry) => (
              <div
                key={entry.id}
                className="flex gap-2 border border-white/10 bg-white/[0.02] px-2 py-1 text-[10px]"
              >
                <span className="text-sentinel-mist/50 shrink-0">
      ...
      <div className="shrink-0 flex items-center justify-between border-t border-white/[0.06] bg-black/60 px-2 h-[18px]">
        ...
        <div className="flex items-center gap-3 text-[9px] font-mono text-sentinel-mist/40">
          {session.metrics.processCount > 0 && (
            <span>{session.metrics.processCount} proc</span>
          )}
          {opLoading && (
